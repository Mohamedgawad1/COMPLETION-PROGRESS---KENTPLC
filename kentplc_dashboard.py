...
<div class="container">
  ...
  <div id="charts" class="panel">
    <div class="grid g2">
      ...
      <div class="card" style="padding:0;overflow:hidden"><h3 style="padding:22px 22px 0">Completion Radar</h3><div style="background:#1a1a2e;border-radius:0 0 14px 14px;padding:20px"><div class="chart-container tall"><canvas id="chartCompMs"></canvas></div></div></div>
...
var dlOpts = { display: function(ctx){ return ctx.dataset.data[ctx.dataIndex] > 0; }, color: '#fff', font: { weight: '700', size: 11 }, formatter: function(v, ctx){ var sum = ctx.chart.data.datasets[0].data.reduce(function(a,b){return a+b},0); var pct = (v/sum*100).toFixed(1); return pct + '%'; } };
var dlOptsNoCenter = { display: false };
...
function initOverview() {
  new Chart(document.getElementById('overviewPie'), {
    ...
    options: { responsive: true, maintainAspectRatio: false, cutout: '68%', plugins: { legend: { position: 'bottom', labels: { padding: 16, usePointStyle: true, pointStyle: 'circle', color: '#6b5e4d' } }, datalabels: dlOpts } }
  });
...
function initMilestones() {
  ...
  new Chart(document.getElementById('msBar'), {
    ...
    options: { responsive: true, maintainAspectRatio: false,
      ...
      plugins: { legend: { position: 'top', labels: { usePointStyle: true, pointStyle: 'circle', padding: 16, color: '#6b5e4d' } }, datalabels: { display: false } }
    }
  });
...
function initPunchList() {
  ...
  new Chart(document.getElementById('plCatChart'), {
    ...
    options: { responsive: true, maintainAspectRatio: false, cutout: '62%', plugins: { legend: { position: 'bottom', labels: { padding: 16, usePointStyle: true, pointStyle: 'circle', color: '#6b5e4d' } }, datalabels: dlOpts } }
  });
...
function initCharts() {
  ...
  new Chart(document.getElementById('chartCompMs'), {
    ...
    }]}, options: { responsive: true, maintainAspectRatio: false, scales: { r: { beginAtZero: true, max: 5, ticks: { stepSize: 1, backdropColor: 'transparent', color: '#9ca3af', font: { size: 11 } }, grid: { color: 'rgba(255,255,255,0.1)' }, angleLines: { color: 'rgba(255,255,255,0.1)' }, pointLabels: { color: '#d1d5db', font: { size: 12, weight: '600' } } } }, plugins: { legend: { display: false }, datalabels: { display: false } } }
  });
